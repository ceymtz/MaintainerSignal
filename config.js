function inputName(name) {
  return `INPUT_${name.toUpperCase().replaceAll('-', '_').replaceAll(' ', '_')}`;
}

function getInput(name, fallback = '') {
  return String(process.env[inputName(name)] ?? fallback).trim();
}
...
export function loadConfig() {
  ...
  if (!['auto', 'always', 'never'].includes(aiReview)) {
    throw new Error('ai-review must be one of: auto, always, never.');
  }

  if (!['none', 'moderate', 'high', 'critical'].includes(failOnRisk)) {
    throw new Error('fail-on-risk must be one of: none, moderate, high, critical.');
  }
