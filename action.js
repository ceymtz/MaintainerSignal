...
function repositoryContext(event) {
  ...
  if (!fullName || !pullNumber) {
    throw new Error('MaintainerSignal only runs for pull_request events.');
  }
...
async function main() {
  ...
  if (config.comment) {
    try {
      ...
      console.warn(`Comment publication skipped: ${error.message}`);
    }
  }
  ...
  if (shouldFail(analysis.risk.level, config.failOnRisk)) {
    process.exitCode = 1;
    console.error(
      `MaintainerSignal risk ${analysis.risk.level} meets or exceeds fail-on-risk ${config.failOnRisk}.`
    );
  }
}
